...
class NodeMathRule:
    ...
    def __init__(self, xpath: str, attributes: dict):
        ...
        # new_attrs.pop("text")
        attr_str = " and ".join([f'@{key}="{value}"' for key, value in new_attrs.items()])
    ...
    def match_node_in_ui(self, all_node_list: list["Node"]) -> "Node|None":
        # resource-id
        node_by_id = None
        ...
        # attributes_xpath
        flag, node_by_attributes_xpath = self._match_only_one(
            all_node_list, lambda n: n.attributes_xpath == self.attributes_xpath
        )
        if flag:
...
class Node:
    ...
    def __eq__(self, other):
        if isinstance(other, Node):
            ...
            if any(
                [
                    self.attributes.get(key) != other.attributes.get(key)
                    for key in ("package", "class", "resource-id", "text", "content-desc", "hint")
                ]
            ):
                return False
    ...
    def to_dict(self) -> dict:
        return {"level": self.level, "attributes": self.attributes}
    ...
    def similar_to(self, node: "Node", tolerance_rate: float, tolerance_count: int) -> bool:
        ...
        for i in range(len1):
            for j in range(len2):
                if not matched[j] and self.children[i].similar_to(node.children[j], tolerance_rate, tolerance_count):
                    matched[j] = True
                    similar_count += 1
    ...
    def similar_to_list_item(self, node: "Node", tolerance_rate: float, tolerance_count: int) -> bool:
        ...
        for i in range(len1):
            for j in range(len2):
                if not matched[j] and self.children[i].similar_to_list_item(
                    node.children[j], tolerance_rate, tolerance_count
                ):
                    matched[j] = True
                    similar_count += 1
    ...
    def find_similar_node_list_in_direct_children(
        self, tolerance_rate: float, tolerance_count: int
    ) -> list[set["Node"]]:
        """
