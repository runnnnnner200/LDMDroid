...
def collect(apk: Apk, max_action_exec: int, base_out_dir: str, temperature: float):
    ...
    collect_all_dmf(apk, max_action_exec, dmf_output_dir, temperature)
...
def test(
    base_out_dir: str, apk: Apk, max_minute: int, temperature: float, start_time: float, skip_validate: bool = False
):
            ...
            if choice == "random":
                ...
                if action.action_type == ActionType.InputTexts:
                    ...
                    action.input_content = StatePropeller.gen_inputs_by_visual_llm(
                        "", cur_info.package, cur_info.activity, [action.node], temperature, None
                    )[0]
                ...
                if not skip_validate:
                    assert reproduce_res is not None
...
def collect_and_test(
    ...
    collect(apk, max_action_exec, base_out_dir, temperature)

    time.sleep(3)

    logger.info("Test with DMF...")
    test(base_out_dir, apk, max_minute, temperature, start_time, skip_validate)
