...
def specific_crud_meanings(package_name: str, temperature: float):
...
- S (Search): Search for a file or folder in the list."""
    ...
    res = LLM.chat_with_image(prompt=prompt, image=image, temperature=temperature)
    ...
    recorder.data["llm"] = {"model": LLM.model_text, "query": prompt, "answer": res, "formatted_answer": formatted_res}
    recorder.data["result"] = formatted_res
    recorder.save()
    return formatted_res


def _state_propel_action(
    sp: StatePropeller,
    ...
    if (
        (sp.crud_type == CRUD.Create or sp.crud_type == CRUD.Update)
        and sp.operation_workflows[sp.op_stage_index].startswith("Enter")
        and sp.action_history[-1][0].action_type != ActionType.InputTexts
    ):
        # input text
        if sp.fill_in_the_blanks_by_visual_llm(
            sp.crud_type.name,
            all_node_list,
            package_name,
            activity_name,
    ...
    if (
        sp.crud_type == CRUD.Search
        and sp.operation_workflows[sp.op_stage_index].startswith("Enter")
        and sp.action_history[-1][0].action_type != ActionType.InputTexts
    ):
        assert sp.target_data is not None
        if sp.fill_in_the_search_blanks_by_visual_llm(
            sp.target_data, all_node_list, package_name, activity_name, temperature, select_recorder
        ):
            action = sp.action_history[-1][0]
            select_recorder.data["result"] = action.to_dict()
            select_recorder.data["result"]["image"] = select_recorder.record_image(Device.screenshot())
    ...
    if action.node is not None:
        select_recorder.data["result"]["image"] = select_recorder.record_image(Device.draw_node_with_bound(action.node))
...
def complete_crud_operation(
    max_action_exec: int, sp: StatePropeller, package_name: str, activity_name: str, temperature: float
):
    """
    :return: -1:  Max action exec reached, 0:  ui not changed, 1:  ui changed
    """
    ...
    while True:
        action_res = _state_propel_action(
            sp,
            Device.get_node_list(),
            max_action_exec,
            package_name,
            activity_name,
            temperature,
...
def back_to_data_list(
    ...
    while True:
        ...
        if action_res == 1:
            ...
            return_recorder.data["result"] = (
                {"msg": "Successfully return to the page with the 'Data list'!"}
                if res is not None
                else {"msg": "Not return to the page with the 'Data list'"}
            )
...
def check_for_dme_by_llm(
    crud_type: CRUD,
    target_data: Node | None,
    old_data_list: list[Node] | None,
    new_data_list: list[Node] | None,
    ...
    for dt in range(test_times):
        ...
        if crud_type == CRUD.Read:
            ...
            res = LLM.chat_with_image(prompt=prompt, image=read_result, temperature=temp)
            formatted_res = LLM.format_to_json(
                res,
                {
            ...
            input_prompt = "Search input related information:\n{}".format(
                "\n".join([f"{i}. {v}." for i, v in enumerate(input_data["ui_changes"], 1)])
            )
            ...
            res = LLM.chat_with_image(prompt=prompt, image=search_result, temperature=temp)
            formatted_res = LLM.format_to_json(
                res,
                {
                    "op_invalid": f"<boolean: answer of 'Is there any logical error in the {crud_type.name} operation?'>",
            ...
            res = LLM.chat(prompt=prompt, temperature=temp)
            formatted_res = LLM.format_to_json(
                res,
                {
                    "op_invalid": f"<boolean: answer of 'Is there any logical error in the {crud_type.name} operation?'>",
...
def dme_report(prompt: str, reason: str, full_res: str, sp: StatePropeller, recorder_start_index: int):
    ...
    doc = [
        ...
        full_res,
        "## Recorder",
        f"{recorder_start_index}.json ~ {recorder_end_index}.json",
    ]
...
def collect_dmf(
    ...
    sp = StatePropeller(
        crud_type, specific_crud, operation_workflows, operation_guidance, determine_stage_guidance, data_group
    )
    ...
    if sp.crud_type != CRUD.Read and sp.crud_type != CRUD.Search:
        new_data_list = back_to_data_list(max_action_exec, sp, apk.package_name, activity_name, temperature)
        if new_data_list is None:
            logger.error("Failed to return to data list, max action exec reached")
            return 0

    op_invalid, reason, llm_res, prompt = check_for_dme_by_llm(
        sp.crud_type,
        sp.target_data,
        sp.data_group,
        new_data_list,
...
def collect_all_dmf(apk: Apk, max_action_exec: int, out_dir: str, temperature: float):
    ...
    specific_cruds = specific_crud_meanings(apk.package_name, temperature)
    log_process = None
...
def reproduce_and_validate_dmf(
    dmf: dict,
    temperature: float,
    before_action_exec: Callable[[Action], None] | None = None,
    after_action_exec: Callable[[int], None] | None = None,
    ...
    for action in actions:
        ...
        if action_type == ActionType.InputTexts:
            ...
            if crud_type == CRUD.Search:
                ...
                input_list = StatePropeller.gen_inputs_by_visual_llm(
                    crud_type.name,
                    cur_info.package,
                    cur_info.activity,
                    edit_node_list,
    ...
    if skip_validate:
        return
    # validate
    return check_for_dme_by_llm(crud_type, target_data, old_data_list, new_data_list, input_data, temperature)
