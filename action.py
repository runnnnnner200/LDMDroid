...
class Action:
    ...
    def __init__(
        self, action_type: ActionType, node: Node | None, input_content: str | None = None, extra_data: Any = None
    ):
    ...
    def to_dict(self) -> dict:
        out: dict = {"action_type": self.action_type.name}
        if self.node:
            out["target_widget"] = (
                self.node.to_txt()
                if self.node.contain_txt()
                else (self.node.to_id_text() if self.node.contain_id() else f"{self.node.tag}()")
            )
...
class ActionAble:
    ...
    def evaluate_action_by_virtual_llm(
        self, previous_ui: Image, current_ui: Image, action: Action, temperature: float, recorder: CollectRecorder
    ) -> str:
        prompt = (
        ...
        res = LLM.chat_with_image_list(prompt=prompt, temperature=temperature, image_list=[previous_ui, current_ui])
    ...
    @staticmethod
    def gen_inputs_by_visual_llm(
        crud_type: str,
        package_name: str,
        activity_name: str,
        edit_node_list: list[Node],
        temperature: float,
        recorder: CollectRecorder | None,
    ) -> list[str]:
        ...
        prompt = prompt.format(package_name, activity_name, len(edit_node_list), "\n".join(details), crud_type)

        res = LLM.chat_with_image(prompt=prompt, temperature=temperature, image=current_ui)
        format_string = (
            "<string[]: all the example input for these EditText fields>"
            if len(edit_node_list) > 1
            else "<string: the example input for this EditText field>"
        )
        formatted_res = LLM.format_to_json(res, {"input": format_string})
    ...
    @staticmethod
    def gen_search_inputs_by_visual_llm(
        target_data: Node,
        crud_type: str,
        package_name: str,
        activity_name: str,
        ...
        res = LLM.chat_with_image(prompt=prompt, temperature=temperature, image=current_ui)
        format_string = (
            "<string[]: all the input for these EditText fields>"
            if len(edit_node_list) > 1
    ...
    def _fill_in_the_blanks(
        ...
        text_changes = []
        node_list = [
            node for node in all_node_list if node.tag.endswith("EditText") or node.tag.endswith("AutoCompleteTextView")
        ]

        if not node_list:
            return False

        input_list = gen_func(crud_type, package_name, activity_name, node_list, temperature, recorder)
        for node, new_text in zip(node_list, input_list):
            ...
            ui_change = (
                f"Change the text in the {node.tag} widget from '{old_text}' to '{new_text}'"
                if old_text
                else f"Insert '{new_text}' into the EditText widget."
            )
            try:
    ...
    def fill_in_the_blanks_by_visual_llm(
        self,
        crud_type: str,
        all_node_list: list[Node],
        package_name: str,
        ...
        return self._fill_in_the_blanks(
            self.gen_inputs_by_visual_llm, crud_type, all_node_list, package_name, activity_name, temperature, recorder
        )

    def fill_in_the_search_blanks_by_visual_llm(
        self,
        target_data: Node,
        all_node_list: list[Node],
        package_name: str,
        ...
        return self._fill_in_the_blanks(
            partial(self.gen_search_inputs_by_visual_llm, target_data),
            "Search",
            all_node_list,
            package_name,
            activity_name,
    ...
    def _select_action_by_visual_llm(
        self,
        all_node_list: list[Node],
        package_name: str,
        activity_name: str,
        ...
        prompt, action_dict, node_list = self._prompt_select_action(
            prompt, all_node_list, package_name, activity_name, loose=True
        )
        ...
        res = LLM.chat_with_image(prompt=prompt, temperature=temperature, image=image)
    ...
    def _prompt_select_action(
        self, prompt: str, all_node_list: list[Node], package_name: str, activity_name: str, loose=False
    ) -> tuple[str, dict[str, Action], list[Node]]:
        action_dict, widget_list, node_list = self._widget_list(all_node_list, loose)
        action_dict, additional_actions = self._additional_actions(all_node_list, action_dict, len(widget_list) + 1)
    ...
    def _widget_list(self, all_node_list: list[Node], loose=False) -> tuple[dict[str, Action], list[dict], list[Node]]:
        ...
        def _post_order(node: Node):
            ...
            if (
                ...
                or node.attributes["long-clickable"] == "true"
    ...
    def _additional_actions(
        self, all_node_list: list[Node], action_dict: dict[str, Action], additional_action_index: int
    ...
    def _action_history(self) -> list[dict]:
        ...
        for action, ui_change in self.action_history:
            obj: dict = {"action type": action.action_type.name, "ui change": ui_change}
            if action.node is not None:
                obj["target widget"] = (
                    action.node.to_txt()
                    if action.node.contain_txt()
                    else (action.node.to_id_text() if action.node.contain_id() else f"{action.node.tag}()")
                )
            output.append(obj)
        return output
