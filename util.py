...
def prompt_in_yaml(data, extra_intend=2) -> str:
    return "\n".join(
        " " * extra_intend + line for line in yaml.safe_dump(data, allow_unicode=True, default_style="|").splitlines()
    )
...
def basic_init():
    ...
    LLM.init(
        env["api_key"],
        env["base_url"],
        model_text=env["model_text"],
        model_visual=env["model_visual"],
        model_format=env["model_text"],
    )
