...
class StatePropeller(ActionAble):
    ...
    def __init__(
        ...
        assert data_list_parent is not None
    ...
    def determine_crud_op_stage(self, package_name: str, temperature: float, recorder: CollectRecorder) -> bool:
        """
        ...
        if self.determine_stage_guidance:
            guidance_prompt = (
                "Additional guidance:\n"
                + "\n".join([f"{index}. {item}" for index, item in enumerate(self.determine_stage_guidance, 1)])
                + "\n\n"
            )
        else:
            guidance_prompt = ""
        ...
        for dt in range(test_times):
            ...
            formatted_res = LLM.format_to_json(
                res, {"step_number": "<int: specific step number of the operation you are about to perform next>"}
            )
    ...
    def select_action_by_visual_llm(
        self,
        ...
        action = self._select_action_by_visual_llm(
            all_node_list, package_name, activity_name, additional_prompt, temperature, recorder
        )
        # set target data
        if (
            self.crud_type != CRUD.Create
            and self.crud_type != CRUD.Search
            and self.target_data is None
            and action.related_data is not None
        ):
